...
export const getProfile = async (userId) => {
  try {
    ...
    console.log("Coded UserId", userId);
    console.log("hardCoded UserId", "69d9069924472d642c2c4b4b");

    if (!user) {
      throw new Error("User not found");
    }

    const EscrowScore = user.escrowScore || 0;
    ...
    const PendingTransactions = await Transaction.countDocuments({
      sellerId: userId,
      transactionStatus: { $in: ["pending", "shipped"] },
    });
