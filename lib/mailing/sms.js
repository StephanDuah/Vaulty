...
export const sendDelayedShippingReminder = async (info, type) => {
  const {
    product,
    buyerName,
    shippingDate,
  ...
  switch (type) {
    ...
    case "seller":
      phoneNumber = formatGhanaPhone(sellerNumber);
      message = `Please you must ship the products ${product} to ${buyerName} by the end of ${shippingDate} to receive an amount of GHS ${totalAmount}`;
      break;
    default:
      break;
  }
...
export const sendUserVeficationMessage = async (phoneNumber, message) => {
  await sendMessage(phoneNumber, message);
};
